#define REV 2
//firmware version update of CEArl from Teensy3.2 to Teensy4.0 (150MHz)
// using now 2 SPI ports/modules
// used in P6Leander and P5 pair
// For Test purpose ifdef P5 is copied and changed to create dummy-data, as no ADC are connected.
// Test the #define STR_SERIAL_NUMBER G-30DA4-R (Example nr for test purpose)
...
//#define IS_LEFT
#include "usb_serial.h" // may be neccassay to activate usb descipors like #define STR_SERIAL_NUMBERL "12345"
...
// Define USB-Descriptors, Read more at https://github.com/loganb/teensy-usb-serial/blob/master/usb_serial.c#L63 
#define STR_SERIAL_NUMBERL "12345"
#define STR_MANUFACTURER  L"Your Name"
#define STR_PRODUCT   L"USB Serial"

// fitted peripherals
#ifdef P5pair // Adapted to just send dummy data as no adc is conneced
...
#ifdef P6Leander // P10rs(ELC) -> check all ifdef P6Leander! e.g. wak, ps0, rst also p6 has 5adc's p10 has 4 (state 2022-02-19)
  //#define NO_ADC
  //#define SIM_ADC
  #define ADC_SPI1
  #define ADC_MAX11131
  //#define BEND_SENSORS // fals for P10rs(ELC)(state 2022-02-19), true for P6 
  //#define HAVE_IMU     // fals for P10rs(ELC)(state 2022-02-19), true for P6
  #define IMU_SPI0 
#endif
...
+ String("IMU_")
#endif
+ String(ARDUINO,DEC) + String("_") + String(REV,DEC) + String("_") + String(__DATE__).replace(" ", "-") + String("_glove_generic_T4-usb.ino");
  ...
  const byte imuINTPin = 17;  //INT
  const byte imuRSTPin = 22;  //RST
  const byte imuCSPin = 36; //CS2 on Teensy4.0
  // CS peripheral config
  const byte peripheralA0Pin = 10; //Device A: ADC MAX11131A    or older ADC: AD7490 The order may be changed !?!!
  const byte peripheralA1Pin = 9;  //The order may be changed !?!!
  const byte peripheralA2Pin = 20; //The order may be changed !?!!
  const byte peripheralA3Pin = 21; //The order may be changed !?!!
  //const byte peripheralA4Pin = 0; //(bens-sensor and Calib-Tool ADC ) // fals for P10rs(ELC)(state 2022-02-19), true for P6 
  byte ADC_CS[NUM_ADC] = {peripheralA0Pin,peripheralA1Pin,peripheralA2Pin,peripheralA3Pin}; //should match number of ADC's ,peripheralA4Pin
#endif

#ifndef NO_ADC
  #ifdef ADC_SPI0
    const byte adcCOPIPin = 11; //MOSI
    const byte adcCIPOPin = 12; //MISO
    const byte adcSCKPin = 13; //SCLK
    SPIClass &adcSPI = SPI;
  #endif
  #ifdef ADC_SPI1
    const byte adcCOPIPin = 26; //MOSI
    const byte adcCIPOPin = 1; //MISO
    const byte adcSCKPin = 27; //SCLK
...
uint16_t AnalogData[NUM_TAXELS+NUM_BEND_SENSORS]; // analog sensor data
    ...
    const byte imuCOPIPin = 11; //MOSI
    const byte imuCIPOPin = 12; //MISP
    const byte imuSCKPin = 13; //SCLK
    SPIClass &imuSPI = SPI;
  #endif
  #ifdef IMU_SPI1
    // SPI1 on Teensy 4.0 uses COPI Pin = 26 CIPO Pin = 1, SCK Pin = 27
    const byte imuCOPIPin = 26; //MOSI
    const byte imuCIPOPin = 1; //MISO
    const byte imuSCKPin = 27; //SCLK
    SPIClass &imuSPI = SPI1;
  #endif
  #ifdef IMU_SPI2
    // SPI2 on Teensy 4.0 uses COPI Pin = 35 CIPO Pin = 34, SCK Pin = 37
    const byte imuCOPIPin = 35; //MOSI
    const byte imuCIPOPin = 34; //MISO
    const byte imuSCKPin = 37; //SCLK
